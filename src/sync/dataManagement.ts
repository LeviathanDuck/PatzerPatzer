...
function reviewGameIdsForAccount(games: StoredGameRecord[], accountIdValue: string): Set<string> {
  return new Set(games.filter(game => game.accountId === accountIdValue).map(game => game.id));
}
...
export async function getDataManagementSnapshot(): Promise<DataManagementSnapshot> {
  ...
  try {
    ...
    for (const game of games) {
      if (game.accountId && !accountsById.has(game.accountId)) {
        accountsById.set(game.accountId, fallbackAccount(game.accountId, games));
      } else if (!game.accountId && game.importedUsername) {
...
export async function deleteAccountReviewData(accountIdValue: string): Promise<DataManagementResult> {
  const mainDb = await openMainDb();
  try {
    const games = await readAll<StoredGameRecord>(mainDb, 'games');
    ...
    const analysisKeys = analysis.filter(item => gameIds.has(item.gameId)).map(item => item.gameId);
    const summaryKeys = summaries.filter(item => gameIds.has(item.gameId)).map(item => item.gameId);
    const retroKeys = retroResults.filter(item => gameIds.has(item.gameId)).map(item => item.gameId);
...
export async function deleteAccountGames(accountIdValue: string): Promise<DataManagementResult> {
  ...
  try {
    ...
    const deletedGames = await deleteMainStoreKeys('games', [...gameIds]);
